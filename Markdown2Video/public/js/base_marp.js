  ...
  async function updateMarpPreview() {
    ...
    try {
      ...
      if (typeof DOMPurify !== "undefined") {
        ...
        console.warn(
          "DOMPurify no está cargado. El HTML de la previsualización se inserta sin saneamiento adicional del lado del cliente."
        );
  ...
  generateButtons.forEach((button) => {
    button.addEventListener("click", async function () {
      ...
      try {
        ...
        if (result.success) {
          if (format === "mp4" && result.videoUrl) {
            ...
            if (videoContainer && videoPlayer && downloadLink) {
              ...
              console.error(
                "No se encontraron los elementos del reproductor de video."
              );
              alert(
                "¡Video generado! No se pudo mostrar el reproductor, pero puedes intentar recargar."
              );
            }
        ...
        alert(
          `Hubo un error al generar el archivo ${format.toUpperCase()}:\n${
            error.message
          }`
        );
      } finally {
        ...
        else if (this.dataset.format === "ppt")
          this.textContent = "Generar PPT";
        else if (this.dataset.format === "mp4")
          this.textContent = "Generar Video MP4";
        else if (this.dataset.format === "html")
          this.textContent = "Generar HTML";
      }
    });
  });
});
